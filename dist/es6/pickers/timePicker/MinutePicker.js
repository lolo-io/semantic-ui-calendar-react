...
var MinutePicker = /** @class */ (function (_super) {
    ...
    MinutePicker.prototype.getSelectableCellPositions = function () {
        ...
        var all = range(0, this.minutesOnPage);
        if (disabled) {
    ...
    MinutePicker.prototype.getDisabledPositions = function () {
        ...
        if (minDate) {
            if (minDate.isSame(this.state.date, 'hour')) {
                disabledByMinDate = concat(disabledByMinDate, range(0, minDate.minute()).map(function (m) { return _this.getMinuteCellPosition(m); }));
            }
        }
        if (maxDate) {
            if (maxDate.isSame(this.state.date, 'hour')) {
                disabledByMaxDate = concat(disabledByMaxDate, range(maxDate.minute() + this.minutesStep, 60).map(function (m) { return _this.getMinuteCellPosition(m); }));
            }
        }
